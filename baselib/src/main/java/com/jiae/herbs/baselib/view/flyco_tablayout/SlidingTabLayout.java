package com.jiae.herbs.baselib.view.flyco_tablayout;
...
import com.jiae.herbs.baselib.view.flyco_tablayout.listener.OnTabSelectListener;
import com.jiae.herbs.baselib.view.flyco_tablayout.utils.UnreadMsgUtils;
import com.jiae.herbs.baselib.view.flyco_tablayout.widget.MsgView;
...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private ArrayList<Integer> mSelectedTextColors;
    private ArrayList<Integer> mUnSelectedTextColors;
    private ArrayList<Boolean> mTextBolds;
    ...
    public void setViewPager(ViewPager vp) {
        ...
        mSelectedTextColors = new ArrayList<>();
        mUnSelectedTextColors = new ArrayList<>();
        mTextBolds = new ArrayList<>();

        this.mViewPager.removeOnPageChangeListener(this);
        this.mViewPager.addOnPageChangeListener(this);
        setupAdapterNotify(vp);
    ...
    private void setupAdapterNotify(ViewPager vp) {
        final PagerAdapter adapter = vp.getAdapter();
        if (adapter != null) {
        ...
        vp.addOnAdapterChangeListener(mAdapterChangeListener);
    }
    ...
    private void addTab(final int position, String title, View tabView) {
        ...
        mSelectedTextColors.add(mTextSelectColor);
        mUnSelectedTextColors.add(mTextUnselectColor);
        mTextBolds.add(false);
    ...
    private void updateTabStyles() {
        for (int i = 0; i < mTabCount; i++) {
            ...
            int textSelectColor = mTextSelectColor;
            int textUnSelectColor = mTextUnselectColor;
            boolean textBold = false;

            if (mSelectedTextColors.size() > i) {
                textSelectColor = mSelectedTextColors.get(i);
            }
            if (mUnSelectedTextColors.size() > i) {
                textUnSelectColor = mUnSelectedTextColors.get(i);
            }
            if (mTextBolds.size() > i) {
                textBold = mTextBolds.get(i);
            }

//            v.setPadding((int) mTabPadding, v.getPaddingTop(), (int) mTabPadding, v.getPaddingBottom());
            TextView tv_tab_title = (TextView) v.findViewById(R.id.tv_tab_title);
            if (tv_tab_title != null) {
                tv_tab_title.setTextColor(i == mCurrentTab ? textSelectColor : textUnSelectColor);
     ...
     * @param color    变换颜色
     */
    public void setTabSelectedColor(int position, @ColorRes int color) {
        if (mSelectedTextColors.size() <= position)
            return;
        mSelectedTextColors.set(position, ContextCompat.getColor(mContext, color));
        updateTabStyles();
    }

    public void setTabSelectedColor(int position, String color) {
        if (mSelectedTextColors.size() <= position)
            return;
        mSelectedTextColors.set(position, Color.parseColor(color));
        updateTabStyles();
    }
     ...
     * @param color    变换颜色
     */
    public void setTabUnSelectedColor(int position, @ColorRes int color) {
        if (mUnSelectedTextColors.size() <= position)
            return;
        mUnSelectedTextColors.set(position, ContextCompat.getColor(getContext(), color));
        updateTabStyles();
    }

    public void setTabUnSelectedColor(int position, String color) {
        if (mUnSelectedTextColors.size() <= position)
            return;
        mUnSelectedTextColors.set(position, Color.parseColor(color));
        updateTabStyles();
    }
     ...
     * @param bold     是否加粗
     */
    public void setTabTextBold(int position, boolean bold) {
        if (mTextBolds.size() <= position)
            return;
        mTextBolds.set(position, bold);
        updateTabStyles();
    }
        ...
        for (int i = 0; i < mTabCount; ++i) {
            ...
            if (tab_title != null) {
                ...
                if (mTextBold == TEXT_BOLD_WHEN_SELECT) {
                    tab_title.getPaint().setFakeBoldText(isSelect || textBold);
                }
            }
        }
    }
    ...
    private SparseArray<Boolean> mInitAlertSetMap = new SparseArray<>();
    ...
    public void showAlert(int position, String msg) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }

        View tabView = mTabsContainer.getChildAt(position);
        MsgView tipView = (MsgView) tabView.findViewById(R.id.rtv_hint_alert);
        if (tipView != null) {
            UnreadMsgUtils.show(tipView, msg);

            if (mInitAlertSetMap.get(position) != null && mInitAlertSetMap.get(position)) {
                return;
            }

            setAlertMargin(position, 2, 2);
            mInitAlertSetMap.put(position, true);
        }
    }
     ...
     * 隐藏Alert
     */
    public void hideAlert(int position) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }

        View tabView = mTabsContainer.getChildAt(position);
        MsgView alertView = (MsgView) tabView.findViewById(R.id.rtv_hint_alert);
        if (alertView != null) {
    ...
    /**
     * 设置Alert偏移,原点为文字的左上角.当控件高度固定,消息提示位置易控制,显示效果佳
     */
    public void setAlertMargin(int position, float rightPadding, float bottomPadding) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
        View tabView = mTabsContainer.getChildAt(position);
        ...
        if (alertView != null) {
            ...
            float textWidth = mTextPaint.measureText(tv_tab_title.getText().toString());
            float textHeight = mTextPaint.descent() - mTextPaint.ascent();
            MarginLayoutParams lp = (MarginLayoutParams) alertView.getLayoutParams();
            lp.leftMargin = mTabWidth >= 0 ? (int) (mTabWidth / 2 - textWidth / 2 - dp2px(rightPadding) - lp.width) : (int) (mTabPadding - dp2px(rightPadding) - lp.width);
            lp.topMargin = mHeight > 0 ? (int) (mHeight - textHeight) / 2 - dp2px(bottomPadding) : 0;
            alertView.setLayoutParams(lp);
        }
    }
    ...
    /**
     * 当前类只提供了少许设置Alert属性的方法,可以通过该方法获取MsgView对象从而各种设置
     */
    public MsgView getAlertView(int position) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
        View tabView = mTabsContainer.getChildAt(position);
        return (MsgView) tabView.findViewById(R.id.rtv_hint_alert);
    }
    ...
    void setPagerAdapter(@Nullable final PagerAdapter adapter, final boolean addObserver) {
        ...
        if (pagerAdapter != null && mPagerAdapterObserver != null) {
            // If we already have a PagerAdapter, unregister our observer
            pagerAdapter.unregisterDataSetObserver(mPagerAdapterObserver);
        }
